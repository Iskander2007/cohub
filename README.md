...
- XSS защита
- Ограничение частоты API-запросов и brute force попыток входа
- Secure cookie и HTTP security headers для production-конфигурации
- Ограничение размера и типа загружаемых аватаров
- Команда резервного копирования данных: `python manage.py backup_data --compress`

### Базовые меры безопасности

- Несанкционированный доступ: все API требуют аутентификацию, cookies помечаются как HttpOnly/SameSite, `ALLOWED_HOSTS` и `CSRF_TRUSTED_ORIGINS` задаются явно.
- Утечки данных: `DEBUG` переводится под управление env-переменной, добавлены заголовки `X-Frame-Options`, `Referrer-Policy` и `nosniff`.
- Кража паролей: Django password validators применяются и при регистрации, и при смене пароля; вход ограничен по количеству неудачных попыток.
- Злоупотребление API: включен DRF throttling для анонимных и авторизованных клиентов.
- SQL-инъекция: проект использует Django ORM и не выполняет raw SQL в пользовательских сценариях.
- Захват сервера: ограничены размер запросов и загрузок, добавлена валидация файлов аватаров.
- Случайная потеря данных: доступна команда резервного копирования `backup_data`; для production стоит запускать её по расписанию или использовать snapshot базы на стороне хостинга.
