...
@cache_page(5)  # Кешируем на 5 секунд для снижения нагрузки
def metrics_endpoint(request):
    ...
    # Получаем системные метрики (каждый замер изолирован).
    _add_system_metrics(current_metrics)
...
def metrics_prometheus_format(request):
    ...
    current_metrics = metrics.get_metrics()
    # get_metrics() возвращает cpu/memory = None — дозаполняем их здесь, иначе
    # Prometheus-эндпоинт (основная цель скрейпа Grafana) никогда не отдавал бы
    # CPU/RAM.
    _add_system_metrics(current_metrics)
