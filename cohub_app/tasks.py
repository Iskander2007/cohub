...
# --- Celery -----------------------------------------------------------------
# Реальная Celery-задача (брокер Redis). Автоматически обнаруживается
# celery-приложением (autodiscover_tasks ищет cohub_app/tasks.py). Логику
# переиспользуем из send_loan_reminder, чтобы оба механизма (Celery и встроенный
# DB-воркер) вели себя одинаково. Если celery не установлен — модуль всё равно
# импортируется (задача просто не регистрируется), и работает DB-воркер.
try:
    from celery import shared_task
except Exception:  # pragma: no cover - окружение без celery
    shared_task = None


if shared_task is not None:

    @shared_task(name='cohub_app.tasks.send_loan_reminder_task', bind=True, max_retries=3)
    def send_loan_reminder_task(self, loan_id):
        """Celery-версия напоминания по долгу.

        Выполняется Celery-воркером (`celery -A cohub_settings worker`). В логах
        воркера видно строку вида `Task cohub_app.tasks.send_loan_reminder_task ...
        succeeded`, подтверждающую выполнение фоновой задачи.
        """
        try:
            return send_loan_reminder(loan_id)
        except Exception as exc:  # noqa: BLE001 — типовой ретрай Celery
            logger.exception('Celery send_loan_reminder_task упала для %s', loan_id)
            raise self.retry(exc=exc, countdown=10)


def dispatch_loan_reminder(loan_id):
    """Единая точка постановки напоминания в фон.

    Если настроен брокер Celery (задан REDIS_URL / CELERY_BROKER_URL) — отправляем
    через Celery; иначе кладём во встроенную DB-очередь (run_worker). Так прод с
    Redis использует именно Celery, а окружения без брокера продолжают работать.
    """
    from django.conf import settings

    use_celery = shared_task is not None and bool(getattr(settings, 'REDIS_URL', None))
    if use_celery:
        send_loan_reminder_task.delay(str(loan_id))
        return 'celery'
    enqueue('send_loan_reminder', loan_id=str(loan_id))
    return 'db-queue'
