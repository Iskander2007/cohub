...
## 14. Краткий сценарий для поддержки

### Если пользователь сообщает, что сайт не работает

1. Проверить статус процесса приложения.
2. Проверить логи и ошибки в консоли.
3. Проверить доступность главной страницы и API.
4. Проверить переменные окружения и настройки хостинга.
5. При необходимости перезапустить приложение.
6. Если проблема сохраняется — восстановить последнюю рабочую версию из backup.

### Если проблема появилась после деплоя

1. Проверить, какие изменения были внесены.
2. Проверить миграции и статические файлы.
3. Сравнить с последней рабочей версией.
4. При необходимости выполнить rollback.
5. Повторно проверить основные сценарии пользователя.

## 15. Рекомендуемая структура для Confluence
